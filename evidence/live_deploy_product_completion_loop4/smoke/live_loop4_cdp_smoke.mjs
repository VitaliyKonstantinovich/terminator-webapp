...
const evidence = path.join(repo, 'evidence/live_deploy_product_completion_loop4');
...
const marker = '20260529-product-loop4-final-experience-v1';
const cacheMarker = 'terminator-mina-pwa-20260529-product-loop4-final-experience-v1';
...
const html = await fetchText(`${liveRoot}?force=loop4-live-smoke-${Date.now()}`);
const sw = await fetchText(`${liveRoot}sw.js?force=loop4-live-smoke-${Date.now()}`);
const manifest = JSON.parse(await fetchText(`${liveRoot}manifest.webmanifest?force=loop4-live-smoke-${Date.now()}`));

const profile = path.join(repo, 'tmp-chrome-live-loop4');
...
try {
  chrome = spawn(chromePath, [
    ...
    `${liveRoot}?force=loop4-live-ui-${Date.now()}`
...
(async () => {
  ...
  const weak = app.runMemorySearch('loop4weakfixture unmatched', { persist: false });
  const memory = {
    ...
    weak_warning: app.memorySearchResultWarning('loop4weakfixture unmatched', weak, app.memorySearchResultQuality(weak, 2))
  };
...
`
  ...
  await cdp.send('Page.navigate', { url: `${liveRoot}?force=loop4-live-mobile-390-${Date.now()}` });
  await Promise.race([cdp.once('Page.loadEventFired'), delay(10000)]);
  await cdp.send('Runtime.evaluate', {
    expression: `new Promise((resolve) => { const check = () => (window.MinaApp && document.readyState !== 'loading') ? resolve(true) : setTimeout(check, 100); check(); })`,
    awaitPromise: true,
  ...
  await writeFile(path.join(smokeDir, 'live_loop4_smoke.json'), JSON.stringify(result, null, 2), 'utf8');
