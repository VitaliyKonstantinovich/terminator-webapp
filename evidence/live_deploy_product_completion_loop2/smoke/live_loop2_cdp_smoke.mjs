...
const evidence = path.join(repo, 'evidence/live_deploy_product_completion_loop2');
...
const marker = '20260529-product-loop2-functional-reality-v1';
const cacheMarker = 'terminator-mina-pwa-20260529-product-loop2-functional-reality-v1';
...
const html = await fetchText(`${liveRoot}?force=loop2-live-smoke-${Date.now()}`);
const sw = await fetchText(`${liveRoot}sw.js?force=loop2-live-smoke-${Date.now()}`);
const manifest = JSON.parse(await fetchText(`${liveRoot}manifest.webmanifest?force=loop2-live-smoke-${Date.now()}`));

const profile = path.join(repo, 'tmp-chrome-live-loop2');
...
try {
  chrome = spawn(chromePath, [
    ...
    `${liveRoot}?force=loop2-live-ui-${Date.now()}`
...
`;
  ...
  await writeFile(path.join(screenshotDir, 'live_loop2_scheme_desktop.png'), Buffer.from(desktopShot.data, 'base64'));

  await cdp.send('Emulation.setDeviceMetricsOverride', { width: 390, height: 950, deviceScaleFactor: 1, mobile: true });
  await cdp.send('Page.navigate', { url: `${liveRoot}?force=loop2-live-mobile-${Date.now()}` });
  await Promise.race([cdp.once('Page.loadEventFired'), delay(8000)]);
  await cdp.send('Runtime.evaluate', {
    expression: `new Promise((resolve) => { const check = () => (window.MinaApp && document.readyState !== 'loading') ? resolve(true) : setTimeout(check, 100); check(); })`,
    awaitPromise: true,
  ...
  await writeFile(path.join(screenshotDir, 'live_loop2_scheme_mobile_390.png'), Buffer.from(mobileShot.data, 'base64'));
  ...
  const result = {
    ...
    mobile390,
    screenshots: {
      desktop: path.join(screenshotDir, 'live_loop2_scheme_desktop.png'),
      mobile390: path.join(screenshotDir, 'live_loop2_scheme_mobile_390.png')
    },
    status: (
  ...
  await writeFile(path.join(smokeDir, 'live_loop2_smoke.json'), JSON.stringify(result, null, 2), 'utf8');
