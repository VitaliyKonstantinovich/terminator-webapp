...
const profile = path.join(repo, 'tmp-chrome-product-loop5');
...
try {
  ...
  chrome = spawn(chromePath, [
    ...
    `http://127.0.0.1:${serverPort}/?force=product-loop5-local`
...
(async () => {
  ...
  const demo = await measure('demo_summary', async () => {
    app.workPreview = app.buildWorkPreview('Loop 5 demo: мини-аудит тестового сайта, план автотеста, проверка, память.', {
    ...
    app.createArtifact(task, 'RESEARCH_BRIEF', 'Research Brief: demo audit', 'Цель: проверить тестовый сайт, определить риски и подготовить автотест.', 'Loop 3 max scenario reused as final demo evidence.', 'loop5-demo');
    app.createArtifact(task, 'BRAIN_COMPARISON', 'Brain Council comparison: demo audit', 'Стратег, Критик и Аналитик согласны: PASS_WITH_RISKS, сначала проверить evidence и memory.', 'Comparison and contradiction evidence are stored in Loop 3 max scenario.', 'loop5-demo');
    app.createArtifact(task, 'DECISION_PASSPORT', 'Decision Passport: accept with risks', 'Решение: принять demo chain как V1 RC evidence, owner-assisted пункты оставить отдельным checklist.', 'Причины: artifacts/evidence/verifier/memory связаны; rollback sandbox доказан; AI API не использовались.', 'loop5-demo');
    app.createArtifact(task, 'VERIFIER_VERDICT', 'Verifier: PASS_WITH_RISKS', 'Verifier подтвердил наличие evidence, risks, next step, no AI API, no secrets.', 'Not checked: real phone APK/PWA postponed until production V2 final, billing dashboards, production rollback.', 'loop5-demo');
    app.createArtifact(task, 'MEMORY_SUMMARY', 'Memory Preview: saved', 'Сохранено: Loop 5 final product experience package, demo chain, owner-assisted checklist.', 'Search terms: Loop 5 final product experience mini audit memory.', 'loop5-demo');
    ...
    return {
      ...
      final_answer: {
        ...
        evidence: ['Loop 3 max scenario JSON', 'Loop 5 screenshots', 'Loop 5 performance JSON', 'D archive'],
  ...
  const strongMemory = await measure('memory_search', async () => app.runMemorySearch('Loop 5 final product experience mini audit memory'));
  const strongQuality = app.memorySearchState.result_quality;
  const impossibleMemory = await measure('memory_search', async () => app.runMemorySearch('zzzxxy impossible ownerless query 918273'));
  const impossibleWarning = app.memorySearchState.last_query_warning;
  if (!app.memorySearchState?.records?.some((record) => record.record_id === 'loop5-weak-fixture')) {
    app.memorySearchState.records.unshift({
      record_id: 'loop5-weak-fixture',
      title: 'Loop 5 weak relevance fixture',
      type: 'decision',
      project_id: 'terminator',
      summary: 'Synthetic local-only fixture for weak relevance search classification.',
      search_text: 'loop5weakfixture',
      created_at: new Date().toISOString(),
      tags: ['qamax', 'visual']
    });
  }
  const weakMemory = await measure('memory_search', async () => app.runMemorySearch('loop5weakfixture unmatched', { persist: false }));
  const weakQuality = app.memorySearchResultQuality(weakMemory, 2);
  const weakWarning = app.memorySearchResultWarning('loop5weakfixture unmatched', weakMemory, weakQuality);
  const memory = {
    strong_count: strongMemory.length,
    strong_quality: strongQuality,
    impossible_count: impossibleMemory.length,
    impossible_warning: impossibleWarning,
    weak_count: weakMemory.length,
    weak_warning: weakWarning,
  ...
  return JSON.stringify({
    marker: '20260529-product-loop5-visual-parity-v1',
...
`
    ...
    expression: `
      (async () => {
        ...
        await window.MinaApp.saveGuardianState({
          ...
          status: 'active',
          last_event: 'Loop 5 visual screenshot reset'
        }, { silent: true });
  ...
  result.screenshots = { start: await capture('loop5_start_screen') };
  await cdp.send('Runtime.evaluate', { expression: `window.MinaApp.go('menu', { immediate: true })`, returnByValue: true });
  result.screenshots = { menu: await capture('loop5_main_menu') };
  result.screenshots.start = path.join(screenshotDir, 'loop5_start_screen.png');
  ...
  result.screenshots.main_menu_notifications = await capture('loop5_main_menu_notifications');
  ...
  result.screenshots.scheme_head = await capture('loop5_scheme_head');
  ...
  result.screenshots.scheme_voice = await capture('loop5_scheme_voice');
  ...
  result.screenshots.scheme_hands = await capture('loop5_scheme_hands');
  ...
  result.screenshots.scheme_diagnost = await capture('loop5_scheme_diagnost');
  await cdp.send('Runtime.evaluate', { expression: `window.MinaApp.go('work', { immediate: true })`, returnByValue: true });
  result.screenshots.work_demo = await capture('loop5_work_demo');
  await cdp.send('Runtime.evaluate', { expression: `window.MinaApp.go('scheme', { immediate: true }); document.querySelector('.scheme-zone-card[data-scheme-zone="head"]')?.click();`, returnByValue: true });
  result.screenshots.brain = await capture('loop5_brain_council');
  await cdp.send('Runtime.evaluate', { expression: `window.MinaApp.go('system', { immediate: true })`, returnByValue: true });
  result.screenshots.system = await capture('loop5_system_diagnost');
  ...
  result.screenshots.mobile_390 = await capture('loop5_mobile_390_scheme', { mobile: true });
  ...
  result.screenshots.mobile_430 = await capture('loop5_mobile_430_scheme', { mobile: true });
  ...
  await writeFile(path.join(evidence, 'loop5_product_experience.json'), JSON.stringify(result, null, 2), 'utf8');
  await writeFile(path.join(perfDir, 'loop5_performance.json'), JSON.stringify(result.perf, null, 2), 'utf8');
  await writeFile(path.join(mobileDir, 'loop5_mobile_smoke.json'), JSON.stringify({ mobile_390: result.mobile_390, mobile_430: result.mobile_430 }, null, 2), 'utf8');
  await writeFile(path.join(logsDir, 'loop5_owner_assisted_checklist.json'), JSON.stringify({
